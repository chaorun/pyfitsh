...
def test():
    ...
    inmask=fits.getdata(f"{P}_mask.fits").astype(np.uint8)
    r=Ficalib().calibrate(sci,mask=inmask,bias=bias,flat=flat)
    cli=fits.getdata(f"{P}_maskout.fits").astype(np.float64)
