...
| | CLI | Cython |
|---|---|---|
| 代码 | `if(sscanf(cmd[4],"%lg",&ref_mag)==1) ps[np].use_ref=1; else ps[np].use_ref=0;` | `p->use_ref = 0` |
| 数据来源 | 文本第 5 列（"-" 表示无 ref） | 硬编码 |
| **差异** | **动态解析 ref_mag 并据此设 use_ref** | **始终为 0** |

### 5. ref_mag

| | CLI | Cython |
|---|---|---|
| 代码 | `ps[np].ref_mag = ref_mag;` (有值) / `ps[np].ref_mag = 0.0;` (无值) | `p->ref_mag = 0.0` |
| **差异** | **解析 raw 文件中的星等值** | **始终为 0** |

### 6. ref_col

| | CLI | Cython |
|---|---|---|
| 代码 | `sscanf(cmd[5],"%lg",&ref_col)` → `ps[np].ref_col = ref_col;` 或 `= 0.0` | `p->ref_col = 0.0` |
| **差异** | **解析 raw 文件中的色指数值** | **始终为 0** |

### 7. ref_err

| | CLI | Cython |
|---|---|---|
| 代码 | **未设置**（realloc 不含初始化，垃圾数据） | `p->ref_err = 0.0` |
...
| **差异** | **支持 CIRCULAR / POLYGON 两种类型**（O 多项式路径等） | **仅 CIRCULAR，rp/da 无法表达多边形的顶点坐标** |
...
细节：
- CLI: flag 解析失败 (r<3) → `rfflux[i].flag=MASK_BAD; rfflux[i].flux=0.0; rfflux[i].fluxerr=0.0;`
- Cython: 无此保护，始终赋值 raw flag 列的值
