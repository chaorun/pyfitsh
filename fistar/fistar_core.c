...
int fistar_search_cy(
    ...
    double *cand_xy, int in_ncand, int cand_ncol, double cand_radius,
    ...
    double *pos_flux, double *pos_ferr)
{
    ...
    sfp.iter_symmetric = iter_symmetric;
    sfp.iter_general = iter_general;
    ...
    if (cand_xy != NULL && in_ncand > 0) {
        ...
        for (ci = 0; ci < in_ncand; ci++) {
            icands[ci].x = cand_xy[ci * cand_ncol];
            icands[ci].y = cand_xy[ci * cand_ncol + 1];
        ...
        case ALG_LNK:
            fprintf(stderr,"ALG_LNK threshold=%.1f flux_th=%.1f skysigma=%.1f\n",threshold,flux_threshold,skysigma);
    ...
    /* ---- fallback: if stars is NULL, convert from cands ---- */
    if (stars == NULL)
        convert_candidates(cands, ncand, &stars, &nstar);
