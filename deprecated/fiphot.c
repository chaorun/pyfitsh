...
******************************************************************************/
 if ( imgname != NULL )
  {	int	is_calc_opt_apert;
	...
	mask=mask_read_from_header(&img->header,img->i.sx,img->i.sy,NULL);
	if ( inmasklist != NULL )
	...
	mask_mark_nans(&img->i,mask,MASK_NAN);
  ...
  {	kernellist	*kl;
	...
	if ( refname != NULL )
	 {	
		...
		mask=mask_read_from_header(&refimg->header,refimg->i.sx,refimg->i.sy,NULL);
		if ( inmasklist != NULL )
	...
	if ( refimg != NULL && mask != NULL )
 	 {	submask=mask_duplicate(mask,refimg->i.sx,refimg->i.sy);
		mask_mask_from_header(submask,&subimg->header,refimg->i.sx,refimg->i.sy,NULL);
	 }
	else	submask=mask_read_from_header(&subimg->header,subimg->i.sx,subimg->i.sy,NULL);
