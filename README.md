...
fitsh C 天文软件管线的 Cython 移植版本。通过 Python 调用 `fiarith`（逐像素算术表达式）、`fistar`（星检测/拟合）、`fiphot`（孔径测光）、`grmatch`（星点匹配）、`fitrans`（图像变换）、`ficonv`（图像卷积/减影）、`firandom`（人工图像生成）、`ficalib`（图像校准）、`fiign`（像素 mask 操作）。
...
2026-06-18 快照。复刻 fitsh 0.9.4 行为，遵循原始设计约束：
...
- 新增 19 个 CLI 对照测试脚本（`test/` 目录 + `runtest.sh`），全部 PASS（2026-06-18）
