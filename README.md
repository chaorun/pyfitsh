...
- 修复 `core.pyx` 中 `_psf_type` 常量偏移 bug（native/integral/circle 赋值均偏 -1，导致 `pbg`/`pamp` 为 nan）
- 修复 ficonv / fiphot 模块 mask 处理：需按 USAGE.md 规范传入 `decode_maskinfo` + NaN 标记
- 新增 19 个 CLI 对照测试脚本（`test/` 目录），全部 PASS（含已知 CLI bug 的对齐处理）
- 定位 origincode CLI bug：`--input-candidates --col-shape 3,4,5` 未读取 cd/ck 列（见 USAGE.md）
