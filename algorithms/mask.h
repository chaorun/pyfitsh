...
#ifndef	__MASK_H_INCLUDED
#define	__MASK_H_INCLUDED	1
...
/* mask_read_from_header():
   ...
   newly allocated mask (which can be free with mask_free()).	     */
char **	mask_read_from_header(fitsheaderset *header,int sx,int sy,char *hdr);

/* mask_mask_from_header():
...
int mask_mask_from_header(char **mask,fitsheaderset *header,int sx,int sy,char *mh);
...
int	mask_export_as_header(fitsheaderset *header,int cl,char **mask,int sx,int sy,char *hdr);
...
/* mask_create_empty():
   ...
   the newly allocated mask (which can be freed with mask_free()).      */
...
char **	mask_expand_false(char **mask,int sx,int sy,int dis,int imv,int smv,int expand_border);

int	mask_expand_logic(char **mask,int sx,int sy,int dis,int imv,int smv,int expand_border);
...
/* mask_create_floyd():
   ...
   The other mask creation parameters are like to mask_create_empty().  */
char ** mask_create_floyd(int sx,int sy,int a,int b,int smv);
...
int	mask_trim(char ***mask,int sx,int sy,int x0,int y0,int nx,int ny,int outer);
