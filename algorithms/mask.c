...
char ** mask_create_empty(int sx,int sy)
{
 char	**mask;
 int	i;	

 if ( sx<=0 || sy<=0 )	return(NULL);
 mask=mask_alloc(sx,sy);
...
static int mask_mask_from_header_native(char **mask,fitsheaderset *header,int sx,int sy,int y0,int y1,char *maskhdr)
{
 char		buff[80],*wp,*sp;
...
int mask_mask_from_header(char **mask,fitsheaderset *header,int sx,int sy,char *maskhdr)
{
 int	r;
 if ( sx<=0 || sy<=0 )	return(1);
 r=mask_mask_from_header_native(mask,header,sx,sy,0,sy,maskhdr);
 return(r);
}

char **mask_read_from_header(fitsheaderset *header,int sx,int sy,char *maskhdr)
{
 char	**mask;
 int	r;
 ...
 r=mask_mask_from_header(mask,header,sx,sy,maskhdr);
 if ( r )
  {	mask_free(mask);
	return(NULL);
  }
...
int mask_mask_line(char *line,fitsheaderset *header,int sx,int sy,int y0,char *maskhdr)
{
 int	r;
 r=mask_mask_from_header_native(&line,header,sx,sy,y0,y0+1,maskhdr);
 return(r);
}

int mask_mask_more_line(char **lines,fitsheaderset *header,int sx,int sy,int y0,int ny,char *maskhdr)
{
 int	r;
 r=mask_mask_from_header_native(lines,header,sx,sy,y0,y0+ny,maskhdr);
 return(r);
}
...
static int mask_scan_horizontal(char **mask,int sx,int sy,block **rblocks,int *rnblock)
{
 block	*blocks,*wb;
...
/*static int mask_join_blocks(block *blocks,int nblock)
{
 int	i,j,fi,x,y,sx;
 for ( i=1 ; i<nblock ; i++ )
  {	sx=blocks[i].sx;
...
int mask_export_as_header(fitsheaderset *header,int clean,char **mask,int sx,int sy,char *maskhdr)
{
 ...
 mask_scan_horizontal(mask,sx,sy,&blocks,&nblock);
 mask_join_blocks(blocks,nblock); 
...
char ** mask_expand_false(char **imask,int sx,int sy,int dis,int imv,int smv,int expand_border)
{
 int	i,j,k,l;
 char	**omask;

 omask=mask_alloc(sx,sy);
...
int mask_expand_logic(char **imask,int sx,int sy,int dis,int imv,int smv,int expand_border)
{
 int	i,j,k,l;
 char	**omask;
...
char **mask_duplicate(char **mask,int sx,int sy)
{
 char 	**nm,**wm;
 nm=mask_alloc(sx,sy);
...
int mask_and(char **mask,int sx,int sy,char **other)
{
 char	*p,*q;
 int	s;
...
int mask_or(char **mask,int sx,int sy,char **other)
{
 char	*p,*q;
 int	s;
 if ( other==NULL )	return(0);
...
char ** mask_create_floyd(int sx,int sy,int a,int b,int svm)
{
 char 	**mask;
...
int mask_mark_nans(image *img,char **mask,int setmask)
{
 int	i,j,sx,sy;
...
static int mask_expand(char ***rmask,int sx,int sy,int nx,int ny,int fill)
{
 char	**mask;
 size_t	ns;
 int	i;
...
static int mask_shrink(char ***rmask,int sx,int sy,int nx,int ny)
{
 char	**mask;
 size_t	ns;
 int	i;
...
int mask_copy_line(char **mask,int tx,int ty,int x0,int y0,char *line,int outer)
{
 if ( y0<0 || y0>=ty )
...
int mask_trim(char ***rmask,int sx,int sy,int x0,int y0,int nx,int ny,int outer)
{
 char	**mask,*line;
 int	tx,ty,i;
 ...
 mask_expand(&mask,sx,sy,tx,ty,outer);
 ...
 if ( x0 != 0 || y0 != 0 )
  {	if ( y0>0 )
	 {	for ( i=0 ; i<ty ; i++ )
		 {	mask_copy_line(mask,tx,ty,x0,i+y0,line,outer);
...
int mask_is_clean(char **mask,int sx,int sy)
{ 
 char	*line;
 int	i;
