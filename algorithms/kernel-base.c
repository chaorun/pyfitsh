...
/* ---- profiling counters (shared by C binary + .so) ---- */
static int _prof_subblock_calls   = 0;
static int prof_subblock_pixels  = 0;
static int prof_subblock_nkern   = 0;
static double prof_subblock_time = 0.0;
static int prof_gauss_calls      = 0;
static int prof_gauss_hsize      = 0;
static double prof_gauss_time    = 0.0;
static int prof_solve_calls      = 0;
static int prof_solve_nvar       = 0;
static double prof_solve_time    = 0.0;

void fitrans_profile_dump(void) {
    fprintf(stderr, "PROFILE subblock: calls=%d max_pixels=%d max_nkern=%d total_time=%.3f s\n",
        _prof_subblock_calls, prof_subblock_pixels, prof_subblock_nkern, prof_subblock_time);
    fprintf(stderr, "PROFILE gauss: calls=%d max_hsize=%d total_time=%.3f s\n",
        prof_gauss_calls, prof_gauss_hsize, prof_gauss_time);
    fprintf(stderr, "PROFILE solve: calls=%d max_nvar=%d total_time=%.3f s\n",
        prof_solve_calls, prof_solve_nvar, prof_solve_time);
}
...
int	kernel_verbose=0;  /* default quiet */
#define	KERNEL_LOG	1
...
int kernel_image_calc_gaussian(kernel *k)
{
 ...
 k->offset=0.0;
 prof_gauss_calls++; if (hsize > prof_gauss_hsize) prof_gauss_hsize = hsize;
 prof_gauss_time += (double)(clock()-_pg0)/CLOCKS_PER_SEC;
 return(0);
}
...
double convolve_point(image *img,kernel *k,int x,int y)
{
 int	i,j,hsize;
 double	w;
 w=0.0;
...
int convolve_image(image *img,char **mask,kernel *k,image *out)
{
 int	i,j,sx,sy;
...
static int fit_kernel_poly_coefficients_subblock(image *refimg,image *img,
	...
	stamp *stamps,int nstamp,kernellist *klist,kernellist *xlist)
{
 ...
 /* profile: count call, track max pixels and nkernel */
 { int _pts = 0; for (b=0;b<nstamp;b++) _pts += stamps[b].sx * stamps[b].sy;
   if (_pts > prof_subblock_pixels) prof_subblock_pixels = _pts;
   if (xlist != NULL && xlist->nkernel > prof_subblock_nkern) prof_subblock_nkern = xlist->nkernel;
   _prof_subblock_calls++; }
 ...
 logmsg(kernel_verbose>=2," -> solving the linear least-squares equation...");
  ...
  prof_solve_calls++; if (nvar > prof_solve_nvar) prof_solve_nvar = nvar;
  prof_solve_time += (double)(clock()-_ps0)/CLOCKS_PER_SEC; }
#if KERNEL_LOG != 0
 logmsg(kernel_verbose>=2,"done.\n");
 ...
 prof_subblock_time += (double)(clock()-_pt0)/CLOCKS_PER_SEC;
...
int fit_kernel_poly_coefficients_block(image *refimg,image *img,
...
int fit_kernel_poly_coefficients_stamp(image *refimg,image *img,
...
int convolve_with_kernel_set_poly(image *refimg,char **mask,kernellist *klist,image *outimg)
{
 int	i,j,sx,sy;
 double	ox,oy,scale;
...
int convolve_with_kernel_set(image *refimg,char **mask,kernellist *klist,image *outimg)
{
 int	r;
...
int convolve_to_subtracted_poly(image *ref,image *img,char **mask,
...
int convolve_to_subtracted(image *ref,image *img,char **mask,
	kernellist *klist,kernellist *xlist,image *outimg)
{
 int	r;
