#!/usr/bin/env python3
"""test_10: ficalib post scale 10000 (mean-normalize to 10000)"""
...
def test():
    ...
    cal=r['data']
    out=cal*10000.0/cal.mean()
    cli=fits.getdata(f"{P}_postscale.fits").astype(np.float64)
    return np.allclose(out,cli,rtol=1e-5,atol=1e-2)
