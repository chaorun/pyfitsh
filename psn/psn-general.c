...
static int __fn_f_rnd  (double *s) { *(s-2)=*(s-2)+(*(s-1)-*(s-2))*random_double();return(0);	}
static int __fn_f_gauss(double *s) { *(s-2)=*(s-2)+(*(s-1))*sqrt(-2.0*log(random_double()))*cos(2.0*X_PI*random_double());return(0);	}
