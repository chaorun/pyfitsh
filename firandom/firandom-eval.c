...
int create_input_list(char *buff,starlistparam *lp,star **rstars,int *rnstar,int sseed)
{
 ...
 double	gls,gld,glk,glf,gle,glp,glis,glid,glik;
  ...
  { if ( isdigit((int)terms[i][0]) )
	...
	for ( j=0 ; j<nstg ; j++ )
	 {	double	lx,ly,li,ls,ld,lk;
		...
		vars[RND_1]=random_double(),
		vars[RND_2]=random_double(),
		vars[RND_3]=random_double(),
		vars[RND_4]=random_double();
