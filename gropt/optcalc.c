...
// 		continue;
// 	 }
// 	...  (glass/lens/focal parsing omitted for brevity)
//   }
//  return(0);
// }

int optcalc_read_optics_from_string(const char *text,struct optics *opt)
{
 char	*buf,*saveptr,*line;
 char	**cmd;
 int	i,n;
 int	nline;

 if ( text==NULL || opt==NULL )
	return(-1);
  ...
  {	nline++;
	...
	if ( strcmp(cmd[0],"glass")==0 && n==3 )
	 {	struct	glass	*g;
		...
		if ( 31<strlen(cmd[1]) )
		 {	free(cmd); free(buf); return(nline);	}
